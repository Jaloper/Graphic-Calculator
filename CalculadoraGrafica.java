...
public class CalculadoraGrafica implements ActionListener {
    ...
    private void calcular(){
        ...
        Assessment v=new Assessment();
        v.addAssessment("x", Integer.parseInt(tfx.getText()));
        v.addAssessment("y", Integer.parseInt(tfy.getText())); v.addAssessment("z", Integer.parseInt(tfz.getText()));
        c=new Calculator(exp,v);
        int res=c.calculate();
    ...
    private void sustituir() {
        ...
        Assessment v=new Assessment();
        v.addAssessment("x", Integer.parseInt(tfx.getText()));
        v.addAssessment("y", Integer.parseInt(tfy.getText()));
        v.addAssessment("z", Integer.parseInt(tfz.getText()));
        c=new Calculator(exp,v);
        c.replaceAll();
    ...
    private static Expression parser(String s){
        if (s.charAt(0) =='(') {
            ...
            if (op=='+') return new Sum (parser(s1),parser(s2));
            else if (op=='-') return new Subtraction (parser(s1),parser(s2));
            else return new Multiplication (parser(s1),parser(s2));
        } else {
            if (s.charAt(0) =='-') {
                if (s.charAt(1)=='('){
                ...
                if ((i1==-1) && (i2==-1) && (i3==-1)){
                    ...
                    if ((i1<i2) && (i1 < i3)) {
                        ...
                        return new Sum(parser(s1),parser(s2));

                    } else if ((i2 < i1) && (i2 < i3)){
                        String s1=s.substring(0,i2);
                        String s2=s.substring(i2+1);
                        return new Multiplication(parser(s1),parser(s2));

                    } else {
                        String s1=s.substring(0,i3);
                        String s2=s.substring(i3+1);
                        return new Subtraction(parser(s1),parser(s2));
